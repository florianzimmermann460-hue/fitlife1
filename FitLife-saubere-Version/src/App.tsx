...
function Health(){return <div><PageTitle title="Gesundheit" desc="Apple-Health-Werte, bereit für die iPhone-Synchronisation."/><div className="integrationNotice"><Watch size={20}/><div><b>Apple Health Connector</b><p>Im Web-Prototyp werden Demo-Werte verwendet. Die echte HealthKit-Synchronisation wird über eine iOS-App angebunden.</p></div><span className="pill">Demo</span></div><div className="grid statsgrid"><Metric icon={<Footprints/>} label="Schritte" value="8.742" sub="heute"/><Metric icon={<HeartPulse/>} label="Herzfrequenz" value="64 bpm" sub="Durchschnitt"/><Metric icon={<Moon/>} label="Schlaf" value="7:48 h" sub="letzte Nacht"/><Metric icon={<Activity/>} label="Gewicht" value="78,4 kg" sub="zuletzt synchronisiert"/></div><section className="card chartcard"><CardHead title="Schritte · letzte 7 Tage"/><div className="bars">{[62,74,48,91,68,83,87].map((v,i)=><div key={i}><i style={{height:`${v}%`}}/><span>{["Fr","Sa","So","Mo","Di","Mi","Do"][i]}</span></div>)}</div></section></div>}
